...
export const VoiceNotePlayer = memo(({ 
  ...
  useEffect(() => {
    return () => {
      ...
      // Only stop global audio if THIS player owns it
  ...
  const handlePlay = async () => {
    ...
    // Clicking while playing acts like WhatsApp stop; next click replays from start with a FRESH Audio().
    if (isPlaying) {
      stopCurrentAudio();
      setIsPlaying(false);
      setCurrentTime(0);
    ...
    try {
      ...
      // Detach any previous listeners we attached
      detachRef.current?.();
      detachRef.current = null;
      lastAudioRef.current = null;

      // Single shared playback: stop previous globally, create fresh Audio, wait for readiness, then play.
      ...
      const onPause = () => {
        // This audio got paused (likely because another voice started)
        if (lastAudioRef.current !== audio) return;
        setIsPlaying(false);
        setIsLoading(false);
      };
      ...
      // playVoice() only resolves after audio.play() succeeds
      setIsPlaying(true);
      setIsLoading(false);
