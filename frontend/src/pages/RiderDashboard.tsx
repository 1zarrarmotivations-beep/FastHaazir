...
import NavigationTopCard from '@/components/rider/NavigationTopCard';

/* ================= PRODUCTION BUILD MARKER ================= */
const BUILD_VERSION = 'PROD-v8.0-STABLE';
/* ============================================================ */
...
const RiderDashboard = () => {
  ...
  const { lastLocation, isTracking, locationStatus } = useRiderLocation(
    riderProfile?.id,
    riderProfile?.is_online || false
  );

  // useRealtimeSync is already called inside useRiderLocation, but we
  // expose locationStatus for the UI to show GPS health indicator
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          vibrate('triple');
          // Show IncomingOrderSheet for the new request
          if (payload.new) {
  ...
  }, [riderProfile?.is_online, notifyNewOrder, vibrate, queryClient]);

  // Also show IncomingOrderSheet when pendingRequests first arrive
  useEffect(() => {
    if (pendingRequests.length > 0 && !incomingRequest) {
      setIncomingRequest(pendingRequests[0]);
  ...
  return (
    <div className="min-h-screen rider-bg pb-24 overflow-x-hidden">
      ...
      <div className="relative z-10 pt-12">
        ...
        <div className="px-4 mt-4">
                  ...
                  {activeDeliveries.map((delivery) => (
                    <RiderOrderRequestCard
                      ...
                      onUpdateStatus={(requestId, status, requestType) => {
                        // TASK 5 — Delivery Completion Security:
                        // Before completing, verify rider is within 100m of dropoff
                        if (status === 'delivered') {
                          const dropLat = delivery.dropoff_lat;
                          const dropLng = delivery.dropoff_lng;
                          const rLat = lastLocation?.lat ?? riderProfile?.current_location_lat;
                          const rLng = lastLocation?.lng ?? riderProfile?.current_location_lng;

                          if (dropLat && dropLng && rLat && rLng) {
                            const R = 6_371_000;
                            const φ1 = (rLat * Math.PI) / 180;
                            const φ2 = (dropLat * Math.PI) / 180;
                            const Δφ = ((dropLat - rLat) * Math.PI) / 180;
                            const Δλ = ((dropLng - rLng) * Math.PI) / 180;
                            const a = Math.sin(Δφ / 2) ** 2 + Math.cos(φ1) * Math.cos(φ2) * Math.sin(Δλ / 2) ** 2;
                            const distMetres = R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));

                            if (distMetres > 100) {
                              toast.error(
                                `You must be within 100m of the dropoff location to complete delivery. You are ${Math.round(distMetres)}m away.`,
                                { duration: 5000 }
                              );
                              return;
                            }
          ...
          {activeTab === 'map' && (
            ...
            >
              {/* NavigationTopCard — overlays map with ETA + destination during active delivery */}
              <AnimatePresence>
                {activeDeliveries.length > 0 && (
                  <NavigationTopCard
                    destination={
      ...
      {/* IncomingOrderSheet — premium full-screen slide-up for new orders */}
      <IncomingOrderSheet
